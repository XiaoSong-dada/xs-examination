...
1. 已在教师端和学生端新增 `network/transport` 模块，并完成模块导出。
2. 已新增 WS 基础封装函数（发送通道与写循环；学生端额外包含连接函数）。
3. 已新增 TCP request-reply 浅封装函数。
4. 已在两端 `protocol.rs` 增加消息编解码 helper。
5. 已将现有 WS/TCP 入口切换为调用封装函数，保持业务语义不变。
6. 已完成教师端与学生端 `cargo check` 编译验证，当前通过。
7. 已把 WS 消息构造进一步收口到两端 `protocol.rs` 的 `build_message(...)`，并在 `ws_server.rs`、`ws_client.rs` 替换调用。
8. 已把 WS 编码路径进一步统一为 `encode_message(...)` helper，减少业务文件中直接序列化细节。
9. 已把教师端 WebSocket 握手细节从 `ws_server.rs` 下沉到 `network/transport/ws_transport.rs` 的 `accept_ws(...)`。
10. 已再次完成教师端与学生端 `cargo check` 编译验证，当前通过。
11. 已把教师端 TCP 请求超时模板下沉到 `network/transport/tcp_request_reply.rs`，业务入口改为调用预置模板。
12. 已完成本轮收口后的教师端与学生端 `cargo check` 编译验证，当前通过。

### 正在进行

1. 继续按计划推进“替换已有业务逻辑”阶段，优先做 WS 业务入口的进一步收口与链路回归。

### 下一步

1. 收口 `ws_server.rs` 与 `ws_client.rs` 中剩余传输细节到 transport 层。
2. 收口 `student_control_client.rs` 与 `control_server.rs` 中剩余传输细节到 TCP 薄层。
3. 增补关键链路回归验证（心跳、开考消息、配置下发 ACK、发卷 ACK）。
4. 补充“手工回归执行记录”小节，沉淀每条链路的实际验证结果。
...
1. 在 `apps/teacher/src-tauri` 执行 `cargo check`。
2. 在 `apps/student/src-tauri` 执行 `cargo check`。
...
1. 启动教师端与学生端应用。
2. 在教师端执行设备连接流程，使学生端建立到教师端的 WS 连接。
3. 观察教师端日志，确认持续收到同一学生的心跳。
...
1. 在教师端触发“开始考试”。
2. 观察学生端是否接收开考消息并将本地考试状态切换为 active。
3. 观察教师端是否可针对目标学生定向发送开考消息。
...
#### 步骤 4：TCP 配置下发 ACK 链路

1. 在教师端执行“下发教师地址配置”。
2. 观察教师端是否收到 `APPLY_TEACHER_ENDPOINTS_ACK`。
3. 观察学生端是否完成地址落库并尝试连接主教师端。
...
1. 在教师端执行“分发试卷”。
2. 观察教师端是否收到 `DISTRIBUTE_EXAM_PAPER_ACK`。
3. 观察学生端是否完成试卷落库。
...
| 项目 | 结果 | 说明 |
|---|---|---|
| 编译回归（教师端） | 通过 | 已执行 `apps/teacher/src-tauri cargo check` |
| 编译回归（学生端） | 通过 | 已执行 `apps/student/src-tauri cargo check` |
| WS 心跳链路 | 待执行 | 需应用运行态联调验证 |
| WS 开考指令链路 | 待执行 | 需应用运行态联调验证 |
| TCP 配置下发 ACK 链路 | 待执行 | 需应用运行态联调验证 |
| TCP 发卷 ACK 链路 | 待执行 | 需应用运行态联调验证 |
